...
static void atom2str(scheme *sc, pointer l, int f, char **pp, int *plen)
{
	...
	if (l == sc->NIL) {
		...
		{
			if (num_is_integer(l)) {
				snprintf(p, STRBUFFSIZE, "%ld",
					 ivalue_unchecked(l));
			} else {
				snprintf(p, STRBUFFSIZE, "%.10g",
					 rvalue_unchecked(l));
		...
		p = sc->strbuff;
		snprintf(p, STRBUFFSIZE, "#<%s PROCEDURE %ld>", procname(l),
			 procnum(l));
	} else if (is_macro(l)) {
		p = "#<MACRO>";
		...
		p = sc->strbuff;
		snprintf(p, STRBUFFSIZE, "#<FOREIGN PROCEDURE %ld>",
			 procnum(l));
...
static void new_slot_spec_in_env(scheme *sc, pointer env, pointer variable,
				 pointer value)
{
...
static void new_slot_spec_in_env(scheme *sc, pointer env, pointer variable,
				 pointer value)
{
	car(env) = immutable_cons(sc, immutable_cons(sc, variable, value),
				  car(env));
}
...
static pointer _Error_1(scheme *sc, const char *s, pointer a)
{
	...
	if (x != sc->NIL) {
		if (a != 0) {
			sc->code = cons(
				sc, cons(sc, sc->QUOTE, cons(sc, (a), sc->NIL)),
				sc->NIL);
		} else {
			sc->code = sc->NIL;
		}
...
static pointer opexe_0(scheme *sc, enum scheme_opcodes op)
{
	...
	case OP_DEF0: /* define */
		if (is_immutable(car(sc->code)))
			Error_1(sc, "define: unable to alter immutable",
				car(sc->code));
	...
	case OP_LET0: /* let */
		sc->args = sc->NIL;
		sc->value = sc->code;
		sc->code = is_symbol(car(sc->code)) ? cadr(sc->code) :
							    car(sc->code);
		s_goto(sc, OP_LET1);
	...
	case OP_LET2: /* let */
		...
		for (x = is_symbol(car(sc->code)) ? cadr(sc->code) :
							  car(sc->code),
		    y = sc->args;
	...
	default:
		snprintf(sc->strbuff, STRBUFFSIZE, "%d: illegal operator",
			 sc->op);
...
static pointer opexe_1(scheme *sc, enum scheme_opcodes op)
{
	...
	case OP_C1STREAM: /* cons-stream */
		sc->args =
			sc->value; /* save sc->value to register sc->args for gc */
		x = mk_closure(sc, cons(sc, sc->NIL, sc->code), sc->envir);
		typeflag(x) = T_PROMISE;
	...
	case OP_SUBSTR: { /* substring */
		...
		if (cddr(sc->args) != sc->NIL) {
			...
			if (index1 > strlength(car(sc->args)) ||
			    index1 < index0) {
				Error_1(sc, "substring: end out of bounds:",
					caddr(sc->args));
			}
		} else {
...
int scheme_init_custom_alloc(scheme *sc, func_alloc malloc, func_dealloc free)
{
	...
	for (i = 0; i < n; i++) {
		if (dispatch_table[i].name != 0) {
			assign_proc(sc, (enum scheme_opcodes)i,
...
{
	scheme_define(sc, sc->global_env, mk_symbol(sc, sr->name),
